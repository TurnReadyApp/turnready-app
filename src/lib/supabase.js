...
export async function signUp({ email, password, name, role, inviteCode, phone, plan }) {
  ...
  const profileData = {
    id: userId, email: email.trim().toLowerCase(), name: name.trim(), role,
    avatar: avatarInitials, phone: phone || null, rating: 5.0,
    jobs_completed: 0, total_earned: 0, stripe_status: 'pending', joined_at: now
  }
  if (role === 'manager') {
...
export function onAuthStateChange(callback) {
  return supabase.auth.onAuthStateChange(async (event, session) => {
    if (session?.user) { const profile = await getCurrentUser(); callback(profile) }
    else { callback(null) }
  })
}
...
export async function getTeamCleaners(managerId) {
  const { data: byManagerId, error: e1 } = await supabase
    .from('users').select('*').eq('role', 'cleaner').eq('manager_id', managerId).order('name')
  if (!e1 && byManagerId && byManagerId.length > 0) return byManagerId
  const { data: mgr } = await supabase.from('users').select('invite_code').eq('id', managerId).single()
  if (mgr && mgr.invite_code) {
    const { data: byCode, error: e2 } = await supabase
      .from('users').select('*').eq('role', 'cleaner').eq('invite_code', mgr.invite_code).order('name')
    if (!e2) return byCode || []
  }
  return []
}
...
export async function getProperties(managerId) {
  const { data, error } = await supabase
    ...
    .eq('manager_id', managerId).order('created_at', { ascending: true })
  if (error) throw error
  return (data || []).map(p => ({
    ...p,
    tasks: p.tasks_data || [], rooms: p.rooms_data || [], inventory: p.inventory_data || [],
    schedule: p.schedule || [], cleanerPhotos: p.cleaner_photos || [],
    linenBagPhotos: p.linen_bag_photos || [], cleanerNotes: p.cleaner_notes || '',
    checkIn: p.check_in || '4:00 PM', checkOut: p.check_out || '11:00 AM',
    sameDay: p.same_day || false, accessCode: p.access_code || '',
    supplyInfo: p.supply_info || '', alarmCode: p.alarm_code || '',
    linenRate: p.linen_rate || 10, linenBags: p.linen_bags || 0,
    totalBeds: p.total_beds || 1, assignedTo: p.assigned_to || null,
  }))
}

export async function createProperty(property) {
  const dbProp = {
    manager_id: property.manager_id, name: property.name, address: property.address,
    type: property.type || 'Airbnb', pay: property.pay || 0,
    bedrooms: property.bedrooms || 1, bathrooms: property.bathrooms || 1,
    photo: property.photo || null, notes: property.notes || '',
    ...
    tasks_data: property.tasks || [], rooms_data: property.rooms || [],
    inventory_data: property.inventory || [], schedule: property.schedule || [],
    cleaner_photos: property.cleanerPhotos || [], linen_bag_photos: property.linenBagPhotos || [],
    cleaner_notes: property.cleanerNotes || '', linen_bags: property.linenBags || 0,
...
export async function getPropertyFull(propertyId) {
  ...
  return {
    ...
    photo: data.photo || null, pay: data.pay || 0,
    bedrooms: data.bedrooms || 0, bathrooms: data.bathrooms || 0, notes: data.notes || '',
    check_in: data.check_in || '', check_out: data.check_out || '',
    checkIn: data.check_in || '', checkOut: data.check_out || '',
    same_day: data.same_day || false, sameDay: data.same_day || false,
    access_code: data.access_code || '', accessCode: data.access_code || '',
    supply_info: data.supply_info || '', supplyInfo: data.supply_info || '',
    alarm_code: data.alarm_code || '', alarmCode: data.alarm_code || '',
    linen_rate: data.linen_rate || 0, linenRate: data.linen_rate || 0,
    total_beds: data.total_beds || 0, totalBeds: data.total_beds || 0,
    linen_bags: data.linen_bags || 0, linenBags: data.linen_bags || 0,
    tasks: data.tasks_data || [], rooms: data.rooms_data || [],
    inventory: data.inventory_data || [], cleanerPhotos: data.cleaner_photos || [],
    linenBagPhotos: data.linen_bag_photos || [], cleanerNotes: data.cleaner_notes || '',
    schedule: data.schedule || [], ical_url: data.ical_url || null, icalUrl: data.ical_url || null,
  }
}
...
export async function getSlots(propertyId) {
  const { data, error } = await supabase.from('schedule_slots').select('*').eq('property_id', propertyId).order('date', { ascending: true })
  if (error) throw error
  return data || []
}
...
export async function getCleanerSlots(cleanerId) {
  const { data, error } = await supabase.from('schedule_slots').select('*, properties(*)')
    .or(`cleaner_id.eq.${cleanerId},cleaner_id2.eq.${cleanerId}`).order('date', { ascending: true })
  if (error) throw error
  return data || []
}
...
export async function getPendingJobs(managerId) {
  const { data, error } = await supabase.from('jobs')
    .select('*, properties!inner(manager_id)').eq('properties.manager_id', managerId)
...
export async function saveGuestReview(review) {
  const { data, error } = await supabase.from('jobs')
    ...
    .eq('id', review.jobId).select().single()
  if (error) throw error
  return data
}
...
export async function getPropertyReviews(propertyId) {
  const { data, error } = await supabase.from('jobs')
    ...
    .eq('property_id', propertyId).not('guest_rating', 'is', null)
...
export function subscribeToJobUpdates(propertyId, callback) {
  return supabase.channel(`jobs:${propertyId}`)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'jobs', filter: `property_id=eq.${propertyId}` },
      payload => callback(payload))
    .subscribe()
}

export function subscribeToSlotUpdates(cleanerId, callback) {
  return supabase.channel(`slots:${cleanerId}`)
    .on('postgres_changes', { event: '*', schema: 'public', table: 'schedule_slots', filter: `cleaner_id=eq.${cleanerId}` },
      payload => callback(payload))
    .subscribe()
}
...
export async function uploadVideoToStorage(bucket, path, base64DataUrl, mimeType) {
  ...
  const { data, error } = await supabase.storage.from(bucket).upload(path, blob, { cacheControl: '3600', upsert: true, contentType: mimeType || 'video/mp4' })
  if (error) throw error
  const { data: { publicUrl } } = supabase.storage.from(bucket).getPublicUrl(path)
  return publicUrl
}
...
export async function uploadImageToStorage(bucket, path, base64DataUrl) {
  ...
  const byteArray = new Uint8Array(byteNumbers)
  const blob = new Blob([byteArray], { type: mimeType })
  const { data, error } = await supabase.storage.from(bucket).upload(path, blob, { cacheControl: '3600', upsert: true, contentType: mimeType })
  if (error) throw error
  const { data: { publicUrl } } = supabase.storage.from(bucket).getPublicUrl(path)
  return publicUrl
}
...
export async function getCleanersByInviteCode(inviteCode) {
  ...
  const { data, error } = await supabase.from('users').select('id, name, email, business_name, invite_code')
    .eq('role', 'manager').eq('invite_code', inviteCode.trim().toUpperCase()).single()
...
export async function subscribeToPush(userId) {
  ...
  try {
    ...
    if (existing) { await savePushSubscription(userId, existing.toJSON()); return existing }
    const vapidPublicKey = import.meta.env.VITE_VAPID_PUBLIC_KEY
    if (!vapidPublicKey) { console.log('[Push] VITE_VAPID_PUBLIC_KEY not set'); return null }
